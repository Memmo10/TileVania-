...
public class PlayerMovement : MonoBehaviour
{
    ...
    {
        if (!isAlive) { return; }
        if (myCapsuleCollider2D.IsTouchingLayers(LayerMask.GetMask("Ladder"))) { myRigidbody.gravityScale = ladderGravityScale; }
        else { myRigidbody.gravityScale = generaleGravityScale;}
        Run();
        FlipSprite();
        ClimbLadder();
        Die();
    ...
    void ClimbLadder ()
    {
        ...
        { myAnimator.SetBool("isClimbing", false); return; }
